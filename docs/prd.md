...
The user clicks "Generate draft from notes." The app sends the notes (and optionally the thread title/topic) to the Claude API and receives a structured first draft — a set of posts, each within the 300-character limit, following the same Threadweaver character-counting rules (URLs = 20, mentions = 15).
